...
Pubblicazione finale [run 34216670126](https://github.com/KilluaZoldich/spesaradar/actions/runs/34216670126) riuscita su `05e31c1`: 1160 record prodotti per ambito e un buono. Confronto integrale del buono pubblico con la capture nativa normalizzata: nessuna differenza; `last_verified_at=2026-09-08T10:24:37.335767Z`. Stato remoto Coop ancora `blocked`, ultimo successo nullo e tutti i metadati del fallimento/cooldown invariati. Il log conferma un record di cache ammesso e nessun timestamp rinnovato; nessun nuovo job Coop avviato. [Evidenza strutturata](coop-voucher-deployment.json).

`PAGES_TEST_URL=https://killuazoldich.github.io/spesaradar/ npm --prefix apps/web run test:pages`: **7 passati in 10,0 s** sul sito pubblico finale, compresi avviso di blocco Coop visibile, beneficio distinto dai prodotti, esclusioni, CAP 41012, selezione Conad unica, Famila/Interspar, persistenza, Axe e mobile senza overflow. [Report](pages-e2e-results.json) e screenshot aggiornati. Questo successo browser dimostra la consultazione della cache, non il ripristino dell'accesso remoto Coop.
