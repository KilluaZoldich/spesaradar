...
### Controllo finale degli ambiti Sigma recenti — 8 settembre 2026

Le due tassonomie collegate ai volantini recenti già acquisiti sono state risolte tramite i GET pubblici dichiarati dall'indice REST: `/wp-json/wp/v2/cedi/31762` restituisce **SIGMA CAMPANIA** (`SCAMP`), `/wp-json/wp/v2/cedi/90217` restituisce **LOMBARDI** (`LOMBC`). Questo identifica i gruppi editoriali dei documenti recenti; non dimostra adesione di un negozio di Carpi. I documenti non vengono importati attribuendoli a Realco o a un ambito nazionale. Il profilo Realco e la ricerca Carpi restano quelli verificati nella sezione precedente. Nessun nuovo accesso al servizio Coop bloccato o al percorso Sigma escluso da robots.
